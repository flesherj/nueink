...
export class IntegrationConfigConverter implements Converter<IntegrationConfigEntity, IntegrationConfig> {
  public toEntity = (domain: IntegrationConfig): IntegrationConfigEntity => {
    return {
      ...
      lastTransactionSyncAt: domain.lastTransactionSyncAt?.toISOString(),
      syncInProgress: domain.syncInProgress,
      syncStartedAt: domain.syncStartedAt?.toISOString(),
  ...
  public toDomain = (entity: IntegrationConfigEntity): IntegrationConfig => {
    return {
      ...
      lastTransactionSyncAt: entity.lastTransactionSyncAt ? new Date(entity.lastTransactionSyncAt) : undefined,
      syncInProgress: entity.syncInProgress,
      syncStartedAt: entity.syncStartedAt ? new Date(entity.syncStartedAt) : undefined,
