...
// Load transactions
const allTransactions = JSON.parse(fs.readFileSync('/tmp/transactions_categorized.json', 'utf8'));
...
const isActualSpending = (tx) => {
  ...
  if (tx.aiCategory && tx.aiCategory.includes('Inflow')) return false;
...
const detectRecurring = (transactions) => {
  ...
  transactions.forEach(tx => {
    const key = `${tx.merchant}|${tx.aiCategory}`;
    if (!groups.has(key)) {
      groups.set(key, []);
    }
...
const analyzeCategorySpending = (transactions) => {
  ...
  transactions.forEach(tx => {
    ...
    if (!categories.has(tx.aiCategory)) {
      categories.set(tx.aiCategory, { total: 0, count: 0 });
    }

    const cat = categories.get(tx.aiCategory);
    cat.total += amount;
    cat.count += 1;
  });
...
const analyzeMerchants = (transactions) => {
  ...
  transactions.forEach(tx => {
    ...
    merchant.count += 1;
    merchant.categories.add(tx.aiCategory);
...
const analyzeMonthlyTrends = (transactions) => {
  ...
  transactions.forEach(tx => {
    ...
    const catTotal = month.categories.get(tx.aiCategory) || 0;
    month.categories.set(tx.aiCategory, catTotal + Math.abs(tx.amount));
  });
